          ...
          <div class="grid gap-4 md:grid-cols-[230px,1fr,260px] items-start">
            ...
            <div class="bg-white rounded-2xl ring-1 ring-slate-200 p-3 space-y-3">
              ...
              <button type="button" @click="addBlock('heading')"
                ...
                <span class="w-6 h-6 rounded-lg bg-[#05727d]/10 text-[#05727d] flex items-center justify-center text-[11px] font-bold">H</span>
                <span>Heading (judul)</span>
              </button>

              <button type="button" @click="addBlock('paragraph')"
                      class="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-xs bg-slate-50 hover:bg-slate-100 text-slate-800 border border-slate-200 active:scale-[.98] transition">
                <span class="w-6 h-6 rounded-lg bg-[#05727d]/10 text-[#05727d] flex items-center justify-center text-[11px] font-bold">P</span>
                <span>Paragraf</span>
              </button>

              <button type="button" @click="addBlock('info_table')"
                      class="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-xs bg-slate-50 hover:bg-slate-100 text-slate-800 border border-slate-200 active:scale-[.98] transition">
                <span class="w-6 h-6 rounded-lg bg-[#05727d]/10 text-[#05727d] flex items-center justify-center text-[11px] font-bold">Tb</span>
                <span>Tabel Info Dokumen</span>
              </button>

              <button type="button" @click="addBlock('checklist')"
                      class="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-xs bg-slate-50 hover:bg-slate-100 text-slate-800 border border-slate-200 active:scale-[.98] transition">
                <span class="w-6 h-6 rounded-lg bg-[#05727d]/10 text-[#05727d] flex items-center justify-center text-[11px] font-bold">CL</span>
                <span>Checklist Langkah Kerja</span>
              </button>

              <button type="button" @click="addBlock('photo_gallery')"
                      class="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-xs bg-slate-50 hover:bg-slate-100 text-slate-800 border border-slate-200 active:scale-[.98] transition">
                <span class="w-6 h-6 rounded-lg bg-[#05727d]/10 text-[#05727d] flex items-center justify-center text-[11px] font-bold">Img</span>
                ...
                <div class="space-y-3" x-ref="canvasList">
                  <template x-for="(b, idx) in builder.blocks" :key="b.id">
                    <div class="group border border-dashed border-slate-200 rounded-xl px-3 py-2.5 relative cursor-pointer bg-white"
                         :class="selectedId === b.id ? 'border-[#05727d] bg-[#05727d]/3' : 'hover:border-[#05727d]/50'"
                        ...
                        <table class="w-full text-[11px]">
                          ...
                          <template x-for="(row, rIdx) in b.rows" :key="rIdx">
                            <tr>
                              ...
                              <td class="align-top text-slate-800">:
                          ...
                          <template x-for="(it, iIdx) in b.items" :key="iIdx">
                            <li class="flex items-start gap-2 text-[11px] text-slate-700">
                              ...
                              <div>
                                ...
                                <div class="text-[10px] text-slate-400" x-text="it.note"></div>
                              </div>
                            </li>
                          </template>
                        </ul>
                      ...
                      {{-- PHOTO GALLERY --}}
                      <div x-show="b.type === 'photo_gallery'">
                        <div class="text-[10px] font-semibold text-slate-500 uppercase tracking-wide mb-1"
                             x-text="b.title || 'Foto Pendukung'"></div>

                        <template x-if="!b.photos || !b.photos.length">
                          <div class="text-[11px] text-slate-400 italic">
                            Belum ada foto di SOP ini.
                          </div>
                        </template>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3" x-show="b.photos && b.photos.length">
                          <template x-for="(ph, pIdx) in b.photos" :key="pIdx">
                            <figure class="border border-dashed border-slate-300 rounded-xl p-2">
                              <div class="aspect-[4/3] rounded-lg bg-slate-50 flex items-center justify-center overflow-hidden">
                                <img :src="photoUrl(ph)"
                                     alt=""
                                     class="w-full h-full object-contain"
                                     @error="$event.target.classList.add('opacity-0')">
                              </div>
                              <figcaption class="text-[10px] text-slate-500 mt-1"
                                          x-text="ph.label || ('Foto ' + (pIdx+1))"></figcaption>
                            </figure>
                          </template>
                        </div>
                      </div>
              ...
              <div class="bg-white rounded-2xl ring-1 ring-slate-200 p-3" x-show="currentBlock()">
                ...
                <template x-if="currentBlock()">
                  <div class="space-y-2">
                    ...
                    {{-- Photo gallery inspector (opsional ubah judul & label) --}}
                    <div x-show="currentBlock().type === 'photo_gallery'" class="space-y-2">
                      <div>
                        <label class="text-[11px] text-slate-600">Judul galeri</label>
                        <input type="text"
                               class="mt-1 w-full rounded-xl border-slate-300 focus:border-[#05727d] focus:ring-[#05727d] text-xs py-1.5"
                               x-model="currentBlock().title" @input="syncBuilderToJson()">
                      </div>

                      <div class="space-y-1" x-show="currentBlock().photos && currentBlock().photos.length">
                        <div class="text-[11px] text-slate-500">Label foto</div>
                        <template x-for="(ph, pIdx) in currentBlock().photos" :key="pIdx">
                          <div class="flex items-center gap-1">
                            <input type="text"
                                   class="flex-1 rounded-lg border-slate-300 focus:border-[#05727d] focus:ring-[#05727d] text-[11px] py-1 px-2"
                                   placeholder="Label foto" x-model="ph.label"
                                   @input="syncBuilderToJson()">
                            <span class="text-[10px] text-slate-400" x-text="'path: ' + (ph.path || '-')"></span>
                          </div>
                        </template>
                      </div>

                      <p class="text-[10px] text-slate-400">
                        Foto diambil otomatis dari SOP saat import (kolom <code>photos</code>).
                      </p>
                    </div>
...
<script>
  function tplForm() {
    return {
      ...
      // base URL untuk storage (buat foto)
      storageBaseUrl: @json(rtrim(asset('storage'), '/')),
        ...
        switch (type) {
          ...
          case 'info_table':
            return {
              ...
              title: 'INFORMASI',
              rows: [
                { label: 'Field', value: 'Isi / placeholder' }
              ]
            };
          case 'checklist':
            return {
              ...
              title: 'Checklist',
              items: [
                { text: 'Item checklist', note: '' }
              ]
            };
          case 'photo_gallery':
            return {
              id: 'blk_' + Date.now() + '_img',
              type: 'photo_gallery',
      ...
      makePhotoItem(ph, idx) {
        if (!ph) ph = {};
        const path = ph.path || ph.url || '';
        let url = '';
        ...
        if (path) {
          if (path.startsWith('http://') || path.startsWith('https://')) {
            ...
            url = this.storageBaseUrl + '/' + path.replace(/^\/+/, '');
          }
        }

        return {
          label: ph.desc || ph.label || `Foto ${idx + 1}`,
          path: path,
          url: url
        };
      },
      ...
      photoUrl(ph) {
        ...
        if (!ph.path) return '';
        if (ph.path.startsWith('http://') || ph.path.startsWith('https://')) return ph.path;
        if (ph.path.startsWith('/storage/')) return ph.path;
        return this.storageBaseUrl + '/' + ph.path.replace(/^\/+/, '');
      },
      ...
      sopBuilderToTemplate(builderSchema, sop) {
        ...
        // Optional: content SOP -> paragraph
        const content = attrs.content || sop.content;
        if (content) {
          blocks.push({
            id: 'blk_content_h',
            type: 'heading',
        ...
        // FOTO dari kolom photos
        let rawPhotos = attrs.photos || sop.photos || null;
        const photosArr = this.parsePhotos(rawPhotos);
        ...
        if (photosArr.length) {
          blocks.push({
            ...
            type: 'photo_gallery',
            title: 'FOTO PENDUKUNG',
            photos: photosArr.map((ph, idx) => this.makePhotoItem(ph, idx)),
          });
        }
